...
interface Order {
	...
	payment: {
		preparation: {
			checkoutcomPaymentSession: unknown;
...
const isValidDeliveryStatus = (status: string): status is DeliveryStatus => {
	return ["awaiting-payment", "in-progress", "failed", "completed"].includes(
		status,
	);
};
...
export function useOrder() {
	...
	const refreshOrder = useCallback(
		async (orderId: string, clientSecret: string) => {
			try {
				const response = await fetch(
					`/api/headless/refresh?orderId=${orderId}&clientSecret=${clientSecret}`,
					{
						method: "POST",
					},
				);
				...
				const error =
					err instanceof Error ? err : new Error("Failed to refresh order");
				setError(error);
				return null;
			}
		},
